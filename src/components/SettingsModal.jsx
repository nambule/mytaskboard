...
import { X, Plus, GripVertical, Edit, Archive, RotateCcw } from 'lucide-react'
...
const SettingsModal = ({ onClose, showSchedulingFields = false, onToggleSchedulingFields }) => {
  ...
  const handleAddCompartment = async () => {
    ...
    if (allCompartments.some(comp => comp.name === newCompartment.trim())) {
  ...
  const handleArchiveCompartment = async (compartmentToArchive) => {
    if (compartments.length <= 1) {
      alert('Vous devez conserver au moins un compartiment actif.')
      return
    }

    const confirmed = confirm(
      `Archiver « ${compartmentToArchive.name} » ?\n\nToutes ses tâches seront marquées comme terminées. Le compartiment disparaîtra du tableau et du planning.`,
    )
    if (!confirmed) return

    setActionLoading(true)
    try {
      await archiveCompartment(compartmentToArchive.id)
      setHasChanges(true)
    } catch (err) {
      alert(`Impossible d’archiver le compartiment : ${err.message}`)
    } finally {
      setActionLoading(false)
    }
  }

  const handleRestoreCompartment = async (compartmentToRestore) => {
    setActionLoading(true)
    try {
      await restoreCompartment(compartmentToRestore.id)
      setHasChanges(true)
    } catch (err) {
      alert(`Impossible de restaurer le compartiment : ${err.message}`)
    } finally {
      setActionLoading(false)
    }
  }
  ...
  const handleSaveEdit = async () => {
    ...
    if (editingValue.trim() !== currentCompartment.name && 
        allCompartments.some(c => c.name === editingValue.trim())) {
        ...
        <div className="p-6 overflow-y-auto flex-1">
          <div className="space-y-6">
            ...
            <div>
                    ...
                    <div className="flex items-center gap-1">
                      {editingId === compartment.id ? (
                        ...
                        <>
                          ...
                          <button
                            onClick={() => handleArchiveCompartment(compartment)}
                            className="p-1 text-slate-400 transition-colors hover:text-amber-600 dark:hover:text-amber-400"
                            title="Archiver"
                            aria-label={`Archiver le compartiment ${compartment.name}`}
                          >
              ...
              {!loading && archivedCompartments.length > 0 && (
                <div className="mt-5 border-t border-slate-200 pt-4 dark:border-slate-600">
                  <div className="mb-2 flex items-center gap-2 text-xs font-bold uppercase tracking-wide text-slate-500 dark:text-slate-400">
                    ...
                    Archivés
                    <span className="rounded-full bg-slate-100 px-1.5 py-0.5 text-[10px] dark:bg-slate-700">
                      {archivedCompartments.length}
                    </span>
                  </div>
                  <div className="space-y-2">
                    {archivedCompartments.map((compartment) => (
                      <div
                        key={compartment.id}
                        className="flex items-center justify-between gap-3 rounded-lg border border-dashed border-slate-300 bg-slate-50/70 px-3 py-2.5 dark:border-slate-600 dark:bg-slate-700/30"
                      >
                        ...
                        <button
                          type="button"
                          onClick={() => handleRestoreCompartment(compartment)}
                          disabled={actionLoading}
                          className="inline-flex shrink-0 items-center gap-1.5 rounded-lg px-2 py-1.5 text-xs font-semibold text-blue-600 transition-colors hover:bg-blue-50 disabled:opacity-50 dark:text-blue-300 dark:hover:bg-blue-950/40"
                        >
